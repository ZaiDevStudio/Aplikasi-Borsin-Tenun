...
async function initPageLogic(pageId, role) {
    ...
    if (dropKar) {
        try {
            ...
            snap.forEach(d => {
                ...
                karyawanDataMap[dt.nama] = { rekening: dt.rekening || '-', bank: dt.bank || 'Bank', nama_rekening: dt.nama_rekening || '', foto_qris: dt.foto_qris || '' };
            });
        } catch (e) { console.error(e); }
    }
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = e.target.querySelector('button'); const oldText = btn ? btn.innerText : 'Simpan';
            ...
            try {
                ...
                else if(pageId==='page-tambah-bukti'){col='bukti_gaji';redirect='bukti_pembayaran.html';hasImage=true;}
                ...
                if (hasImage) {
                    const fileEl = document.querySelector('#foto');
                    if (fileEl && fileEl.files[0]) formData.foto = await resizeImage(fileEl.files[0]);
                }
                if (pageId === 'page-tambah-karyawan') {
                    const qrisEl = document.querySelector('#foto_qris');
                    if (qrisEl && qrisEl.files[0]) formData.foto_qris = await resizeImage(qrisEl.files[0]);
                }
                ...
                if (pageId === 'page-hitung-gaji') {
                    const p = parseNumber(formData.pakan); const j = parseNumber(formData.jumlah);
                    formData.pakan = p * j; 
                    if (formData.nama_karyawan && karyawanDataMap[formData.nama_karyawan]) {
                        const k = karyawanDataMap[formData.nama_karyawan];
                        formData.rekening = k.rekening; formData.bank = k.bank; 
                        formData.nama_rekening = k.nama_rekening; formData.foto_qris = k.foto_qris;
                    }
                }
                ...
                if (editId) {
                    if(!formData.foto && !pageId.includes('pesanan') && !pageId.includes('terjual') && !pageId.includes('bukti')) delete formData.foto;
                    if(!formData.foto_qris) delete formData.foto_qris;
            ...
            } catch (err) { alert("Gagal: " + err.message); if(btn) { btn.innerText = oldText; btn.disabled = false; } }
        });
    }
    ...
    if (container || tbody) {
        ...
        else if(pageId==='page-bukti-pembayaran'){ col='bukti_gaji'; renderFunc=renderBukti; }
        ...
        if (tbody) { 
            ...
            snap.forEach(doc => {
                const d = doc.data(); 
                const pakan = parseNumber(d.pakan); const jml = parseNumber(d.jumlah); const gaji = parseNumber(d.gajiBersih);
                ...
                tbody.innerHTML += `<tr><td>${no++}</td><td style="text-align:left;">${d.nama_karyawan}</td><td>${d.nama_ulos}<br><small>${jml} lbr</small></td><td style="color:red">${formatRupiah(pakan)}</td><td>${formatRupiah(potLain)}</td><td style="font-weight:bold;color:green">${formatRupiah(gaji)}</td>${role==='admin'?`<td><button class="btn-danger btn-sm" onclick="hapus('gaji','${doc.id}')">X</button></td>`:''}</tr>`;
            });
            ...
            if(document.getElementById('info_ulos')) document.getElementById('info_ulos').innerText = tU+" Lbr"; 
...
// UPDATE: Render Pesanan agar ada gambar
function renderPesanan(d, adm) { 
    const dt=d.data(); 
    return createCard(dt.foto, `<h4>${dt.nama_ulos}</h4><p>Jml: <strong>${dt.jumlah}</strong> | Oleh: ${dt.nama_karyawan}</p>`, adm, 'pesanan', d.id, 'tambah_pesanan.html'); 
}
...
function renderBukti(d, adm) { const dt=d.data(); const tgl=dt.timestamp?dt.timestamp.toDate().toLocaleDateString():'-'; return createCard(dt.foto, `<h4>${dt.nama_karyawan}</h4><p>Tgl: ${tgl}</p><p style="font-style:italic;">"${dt.keterangan||'-'}"</p><p style="color:green;font-weight:bold;">✅ Terkirim</p>`, adm, 'bukti_gaji', d.id, 'tambah_bukti.html'); }
function renderSlipGaji(d, adm) { 
    ...
    let qrDisplay = ''; if (dt.foto_qris) { qrDisplay = `<img src="${dt.foto_qris}" style="width:120px; height:120px; border:1px solid #ddd; padding:2px; margin:5px 0; cursor:pointer;" onclick="viewImage('${dt.foto_qris}')"> <br><small style="color:green; font-weight:bold;">SCAN UNTUK BAYAR</small>`; } else if (dt.rekening && typeof QRious !== 'undefined') { const qrData = new QRious({ value: dt.rekening, size: 100 }).toDataURL(); qrDisplay = `<img src="${qrData}" style="width:60px; height:60px; border:1px solid #ddd; padding:2px; margin:5px 0;">`; }
    const rek = dt.rekening ? `<div style="margin-top:10px;padding-top:5px;border-top:1px dashed #ccc;text-align:center;"><p style="font-size:0.7rem;color:#888;margin:0;">KODE BAYAR / VA:</p><strong style="color:#000;">${dt.bank}</strong><br><span style="font-size:1.3rem;letter-spacing:1px;font-weight:bold;color:#000;">${dt.rekening}</span>${anInfo} <i class="fas fa-copy" onclick="salin('${dt.rekening}')" style="cursor:pointer;color:blue;margin-left:5px;"></i><br>${qrDisplay}</div>` : '';
...
window.salin = (t) => { navigator.clipboard.writeText(t).then(()=>alert("Disalin: "+t