...
const parseNumber = (val) => { if (!val) return 0; if (typeof val === 'number') return val; const cleanStr = String(val).replace(/[^0-9.-]+/g, ""); return Number(cleanStr) || 0; };
...
function renderBottomNav(pageId) {
    ...
    const navHTML = `<nav class="bottom-nav"><a href="dashboard.html" class="nav-item ${pageId==='page-dashboard'?'active':''}"><i class="fas fa-home"></i><span>Home</span></a><a href="pesanan.html" class="nav-item ${pageId==='page-pesanan'||pageId==='page-tambah-pesanan'?'active':''}"><i class="fas fa-clipboard-list"></i><span>Pesanan</span></a><a href="data_karyawan.html" class="nav-item ${pageId==='page-data-karyawan'||pageId==='page-tambah-karyawan'?'active':''}"><i class="fas fa-users"></i><span>Karyawan</span></a><a href="slip_gaji.html" class="nav-item ${pageId==='page-slip-gaji'||pageId==='page-hitung-gaji'||pageId==='page-rekapan-gaji'?'active':''}"><i class="fas fa-wallet"></i><span>Keuangan</span></a></nav>`;
    document.body.insertAdjacentHTML('beforeend', navHTML);
}
function setupImageModal() {
    if(!document.getElementById('imgModal')) document.body.insertAdjacentHTML('beforeend', `<div id="imgModal" style="display:none;position:fixed;z-index:10002;left:0;top:0;width:100%;height:100%;background:rgba(0,0,0,0.9);justify-content:center;align-items:center;"><span style="position:absolute;top:20px;right:30px;color:white;font-size:40px;cursor:pointer;" onclick="closeImage()">&times;</span><img id="imgModalContent" style="max-width:95%;max-height:95%;border:2px solid white;border-radius:5px;"></div>`);
}
...
async function initPageLogic(pageId, role) {
    ...
    let ulosDataMap = {}; let karyawanDataMap = {}; 
    ...
    if (dropKar) {
        ...
        snap.forEach(d => { const dt=d.data(); dropKar.innerHTML+=`<option value="${dt.nama}">${dt.nama}</option>`; karyawanDataMap[dt.nama]={rekening:dt.rekening||'-',bank:dt.bank||'-'}; });
    }

    if (pageId === 'page-hitung-gaji') {
        const btnHitung = document.getElementById('btnHitung');
        if (btnHitung) btnHitung.addEventListener('click', () => {
            const upah = Number(document.getElementById('upah_lembar').value) || 0;
            const jumlah = Number(document.getElementById('jumlah').value) || 0;
            const pakan = Number(document.getElementById('pakan').value) || 0;
            const pot = (Number(document.getElementById('pot_air').value)||0) + (Number(document.getElementById('pot_wifi').value)||0) + (Number(document.getElementById('pot_stell').value)||0) + (Number(document.getElementById('pot_lain').value)||0);
            const totalPakan = pakan * jumlah; // Rumus: Pakan/lembar * Jumlah
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            e.preventDefault(); const btn = e.target.querySelector('button'); const old=btn.innerText; btn.innerText="Menyimpan..."; btn.disabled=true;
            try {
                ...
                const formData = {};
                e.target.querySelectorAll('input, select').forEach(el => { if(el.id&&el.type!=='submit'&&el.type!=='file'&&el.id!=='gaji_bersih_tampil') formData[el.id]=el.value; });
                if(hasImage) { const f=document.querySelector('input[type="file"]'); if(f&&f.files[0]) formData.foto=await resizeImage(f.files[0]); }
                const imgPrev = document.getElementById('preview_foto'); if((pageId.includes('pesanan')||pageId.includes('terjual'))&&imgPrev&&imgPrev.src) formData.foto=imgPrev.src;
                
                if(pageId==='page-hitung-gaji') {
                    formData.pakan = (Number(formData.pakan)||0)*(Number(formData.jumlah)||0); // Simpan Total Pakan
                    if(formData.nama_karyawan && karyawanDataMap[formData.nama_karyawan]) { formData.rekening=karyawanDataMap[formData.nama_karyawan].rekening; formData.bank=karyawanDataMap[formData.nama_karyawan].bank; }
                }
                ...
                if(pageId==='page-tambah-terjual') {
                    ...
                    if(info.stock<Number(formData.jumlah)) throw new Error("Stok Kurang!");
                    await updateDoc(doc(db,'ulos',info.id),{stock:info.stock-Number(formData.jumlah)});
                    formData.total_harga=Number(formData.jumlah)*info.harga;
                }

                const urlParams = new URLSearchParams(window.location.search); const editId = urlParams.get('id');
                if(editId) { if(!formData.foto && !pageId.includes('pesanan') && !pageId.includes('terjual')) delete formData.foto; await updateDoc(doc(db, col, editId), formData); alert("Update Berhasil!"); }
                else { await addDoc(collection(db, col), formData); alert("Simpan Berhasil!"); }
                window.location.href = redirect;
            } catch(err) { alert("Gagal: "+err.message); btn.innerText=old; btn.disabled=false; }
        });
    }
    ...
    if(container||tbody) {
        ...
        if(container) { container.innerHTML=''; if(snap.empty) container.innerHTML='<p style="text-align:center; margin-top:20px;">Belum ada data.</p>'; else snap.forEach(d=>container.appendChild(renderFunc(d, role==='admin'))); }
        if(tbody) {
            let tG=0, tP=0, tU=0, no=1; tbody.innerHTML='';
            snap.forEach(doc => {
                const d=doc.data(); const p=parseNumber(d.pakan); const j=parseNumber(d.jumlah); const g=parseNumber(d.gajiBersih);
                const pl=parseNumber(d.pot_air)+parseNumber(d.pot_wifi)+parseNumber(d.pot_stell)+parseNumber(d.pot_lain);
                tG+=g; tP+=p; tU+=j;
                tbody.innerHTML+=`<tr><td>${no++}</td><td style="text-align:left;">${d.nama_karyawan}</td><td>${d.nama_ulos}<br><small>${j} lbr</small></td><td style="color:red">${formatRupiah(p)}</td><td>${formatRupiah(pl)}</td><td style="font-weight:bold;color:green">${formatRupiah(g)}</td>${role==='admin'?`<td><button class="btn-danger btn-sm" onclick="hapus('gaji','${doc.id}')">X</button></td>`:''}</tr>`;
            });
...
function createCard(img, info, isAdmin, col, id, page) {
    ...
    div.innerHTML=`${imgHTML}<div class="data-info">${info}</div>${isAdmin ? `<div class="admin-actions"><a href="${page}?id=${id}" class="btn-warning btn-sm" style="text-align:center;">✏️</a><button class="btn-danger btn-sm" onclick="hapus('${col}','${id}')">🗑️</button></div>`:''}`;
    return div;
}
...
function renderKaryawan(d, adm) { const dt=d.data(); let wa="#"; if(dt.hp) wa=`https://wa.me/${dt.hp.toString().replace(/\D/g,'').replace(/^0/,'62')}`; return createCard(dt.foto, `<h4>${dt.nama}</h4><p><a href="${wa}" style="color:green;text-decoration:none;"><i class="fab fa-whatsapp"></i> Chat WA</a></p><p style="font-size:0.8rem;background:#f5f5f5;padding:5px;border-radius:5px;">${dt.bank} - <strong>${dt.rekening}</strong> <i class="fas fa-copy" onclick="salin('${dt.rekening}')" style="cursor:pointer;"></i></p>`, adm, 'karyawan', d.id, 'tambah_karyawan.html'); }
